...
void eb_buf_random_void(void *const buf, const uint32_t sizeBuf) {
    ...
    for (uint32_t i = 0; i < sizeBuf; i++) buffer[i] = (uint8_t)(rand() % 256);
}
...
void eb_buf_random_s16_to_bd(int16_t *const buf, const uint32_t sizeBuf,
    ...
    for (uint32_t i = 0; i < sizeBuf; i++) buf[i] = (rand() & 1) ? max : -max;
}
...
void eb_buf_random_u16_to_0_or_1023(uint16_t *const buf, const uint32_t sizeBuf) {
    for (uint32_t i = 0; i < sizeBuf; i++)
...
void eb_buf_random_u16_to_bd(uint16_t *const buf, const uint32_t sizeBuf,
    ...
    for (uint32_t i = 0; i < sizeBuf; i++) buf[i] = max;
}
...
void eb_buf_random_u32_with_max(uint32_t *const buf, const uint32_t sizeBuf,
    ...
    for (uint32_t i = 0; i < sizeBuf; i++) buf[i] %= (max + 1);
}
...
void eb_buf_random_ptrdiff_with_max(ptrdiff_t *const buf, const uint32_t sizeBuf,
    const ptrdiff_t max) {
    eb_buf_random_void(buf, sizeof(*buf) * sizeBuf);
...
EbBool name(const type *const buf1, const type *const buf2,                   \
...
{                                                                              \
    ...
    for (uint32_t i = 0; i < bufSize; i++) {                                   \
        if (buf1[i] != buf2[i]) {                                              \
            printf("\nbuf1[%3d] = 0x%8x\tbuf2[%3d] = 0x%8x", i, buf1[i], i,    \
                   buf2[i]);                                                   \
            result = EB_FALSE;                                                 \
        }                                                                      \
    }                                                                          \
...
EbBool name(const type *buf1, const type *buf2, const size_t width,             \
...
void eb_unit_test_log_double(const char *const nameBuf,
    const double *const buf, const uint32_t sizeBuf) {
    printf("%16s = ", nameBuf);

    if (sizeBuf == 1) printf("%f\n", buf[0]);
    else {
        for (uint32_t i = 0; i < sizeBuf; i++) {
            printf("%10f,", buf[i]);
            if (!((i + 1) % 32)) printf("\n  ");
...
void name(const char *const nameBuf, const type *const buf,                    \
