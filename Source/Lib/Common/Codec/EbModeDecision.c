    ...
    for (bipredIndex = 0; bipredIndex < BIPRED_3x3_REFINMENT_POSITIONS; ++bipredIndex)
    {
        ...
        ************* */
        ...
        if (context_ptr->injected_mv_count_l0 == 0 || is_already_injected_mv_l0(context_ptr, to_inject_mv_x, to_inject_mv_y) == EB_FALSE) {
            ...
            candidateArray[canTotalCnt].tx_depth = 0;
#endif
            ...
            for (tx_itr = 0; tx_itr < MAX_TXB_COUNT; tx_itr++) {
                candidateArray[canTotalCnt].transform_type[PLANE_TYPE_Y][tx_itr] = DCT_DCT;
                candidateArray[canTotalCnt].transform_type[PLANE_TYPE_UV][tx_itr] = DCT_DCT;
            }
#else
    ...
    uint32_t                         *candTotCnt)
{
    ...
    if (rf[1] == NONE_FRAME)
    {
        ...
        if (inj_mv) {
...
#if TXS_ADDED_AS_MD_CAND
            candidateArray[canIdx].tx_depth = 0;
#endif
            ...
            uint8_t tx_itr;
            for (tx_itr = 0; tx_itr < MAX_TXB_COUNT; tx_itr++) {
                candidateArray[canIdx].transform_type[PLANE_TYPE_Y][tx_itr] = DCT_DCT;
                candidateArray[canIdx].transform_type[PLANE_TYPE_UV][tx_itr] = DCT_DCT;
            }
#else
...
void  inject_intra_candidates_ois(
    ...
    for (intra_candidate_counter = 0; intra_candidate_counter < total_intra_luma_mode; ++intra_candidate_counter) {
        ...
        if (av1_is_directional_mode((PredictionMode)intra_mode)) {
...
#if TXS_ADDED_AS_MD_CAND
            candidate_array[can_total_cnt].tx_depth = 0;
#endif
            ...
            uint8_t tx_itr;
            for (tx_itr = 0; tx_itr < MAX_TXB_COUNT; tx_itr++) {
                candidate_array[can_total_cnt].transform_type[PLANE_TYPE_Y][tx_itr] = DCT_DCT;

                if (candidate_array[can_total_cnt].intra_chroma_mode == UV_CFL_PRED)
                    candidate_array[can_total_cnt].transform_type[PLANE_TYPE_UV][tx_itr] = DCT_DCT;
                else
                    candidate_array[can_total_cnt].transform_type[PLANE_TYPE_UV][tx_itr] =
                    av1_get_tx_type(
                        context_ptr->blk_geom->bsize,
                        0,
    ...
    uint32_t                       *cand_cnt)
{
    ...
    for (dv_i = 0; dv_i < num_dv_cand; dv_i++)
    {
...
#if TXS_ADDED_AS_MD_CAND
        candidateArray[*cand_cnt].tx_depth = 0;
#endif
        ...
        uint8_t tx_itr;
        for (tx_itr = 0; tx_itr < MAX_TXB_COUNT; tx_itr++) {
            candidateArray[*cand_cnt].transform_type[PLANE_TYPE_Y][tx_itr] = DCT_DCT;
            candidateArray[*cand_cnt].transform_type[PLANE_TYPE_UV][tx_itr] = DCT_DCT;
        }
#else
...
#if TXS_ADDED_AS_MD_CAND
void  inject_intra_candidates_tx(
    PictureControlSet            *picture_control_set_ptr,
    ModeDecisionContext          *context_ptr,
    const SequenceControlSet     *sequence_control_set_ptr,
    LargestCodingUnit            *sb_ptr,
                    ...
                    if (!disable_z2_prediction || (p_angle <= 90 || p_angle >= 180)) {
...
#endif
#if TXS_ADDED_AS_MD_CAND
                        candidateArray[canTotalCnt].tx_depth = 1;
#endif
                        ...
                        for (tx_itr = 0; tx_itr < MAX_TXB_COUNT; tx_itr++) {
                            ...
                            if (candidateArray[canTotalCnt].intra_chroma_mode == UV_CFL_PRED)
                                ...
                                candidateArray[canTotalCnt].transform_type[PLANE_TYPE_UV][tx_itr] =
                                av1_get_tx_type(
                                    context_ptr->blk_geom->bsize,
                                    0,
