...
static void write_tx_size_vartx(MacroBlockD *xd, const MbModeInfo *mbmi,
    ...
    int blk_col, FRAME_CONTEXT *ec_ctx, aom_writer *w) {
...
void code_tx_size(
    ...
    uint32_t txfm_context_above_index = get_neighbor_array_unit_top_index(
        txfm_context_array,
        cu_origin_x);
    TX_MODE tx_mode = pcsPtr->parent_pcs_ptr->tx_mode;
    ...
    xd->above_txfm_context = &txfm_context_array->topArray[txfm_context_above_index];
    xd->left_txfm_context = &txfm_context_array->leftArray[txfm_context_left_index];
    const TxSize tx_size = blk_geom->txsize[0];
    //if (pcsPtr->picture_number == 8 && cu_origin_y >> 2 == 6 && cu_origin_x >> 2 == 0)
    //    printf("\n");
    av1_code_tx_size(
        cu_ptr,
        ec_ctx,
        ...
        skip);
    // AMIR
    //printf("%d\t%d\t%d\t%d\n", pcsPtr->picture_number, cu_origin_y >> 2, cu_origin_x >> 2, w->ec.rng);
}
#endif
/************************************
******* Av1EncodeTuCoeff
...
#if TX_SIZE_SEARCH_LEVELS
    NeighborArrayUnit_t     *txfm_context_array = picture_control_set_ptr->txfm_context_array;
#endif
    const BlockGeom         *blk_geom = get_blk_geom_mds(cu_ptr->mds_idx);
