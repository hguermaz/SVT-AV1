...
// 11 |                           11x11 11x21 | 11x02 11x12 11x22 | (Middle Rows)
// 21 |                                 21x21 | 21x02 21x12 21x22 |
//    |                                       --------------------|
// 02 |                                         02x02 02x12 02x22 | (Top Edge)
// 12 |                                               12x12 12x22 | (Bottom Row)
// 22 |                                                     22x22 |
//    -------------------------------------------------------------
...
static INLINE void sub_avg_block(const uint8_t *src, const int32_t src_stride,
    const uint8_t avg, const int32_t width, const int32_t height, int16_t *dst,
...
void av1_compute_stats_base(int32_t wiener_win, const uint8_t *dgd,
    ...
    const int32_t wiener_win2 = wiener_win * wiener_win;
    const int32_t wiener_halfwin = wiener_win >> 1;
    const uint8_t avg = find_average(dgd, h_start, h_end, v_start, v_end, dgd_stride);
    ...
    sub_avg_block(src + v_start * src_stride + h_start, src_stride, avg, width, height, s, s_stride);
    sub_avg_block(dgd + (v_start - wiener_halfwin) * dgd_stride + h_start - wiener_halfwin, dgd_stride, avg,
    ...
    // (There is only 1 square in this step for TAP 3, but there are many for TAP 5 and 7.)
    // Demostration of TAP 3:
    //                       (Left Edge)         (Left Edge)
    //  \ lj   00    10    20      01    11    21      02    12    22
    // ki -------------------------------------------------------------
        ...
        for (j = i + 1; j < wiener_win; j++) {
            for (k = 1; k < wiener_win; k++) {
                for (l = 1; l < wiener_win; l++) {
                    ...
                    for (x = 0; x < width; x++) {
                        delta -= d[(k - 1) * d_stride + i + x] * d[(l - 1) * d_stride + j + x];
                        delta += d[(k - 1 + height) * d_stride + i + x] * d[(l - 1 + height) * d_stride + j + x];
                    }

                    H[(i * wiener_win + k) * wiener_win2 + j * wiener_win + l] = H[(i * wiener_win + k - 1) * wiener_win2 + j * wiener_win + l - 1] + delta;
                }
            }
        }
    }
    ...
    for (i = 0; i < wiener_win; i++) {
        for (k = 1; k < wiener_win; k++) {
            for (l = k; l < wiener_win; l++) {
                ...
                for (x = 0; x < width; x++) {
                    delta -= d[(k - 1) * d_stride + i + x] * d[(l - 1) * d_stride + i + x];
                    delta += d[(k - 1 + height) * d_stride + i + x] * d[(l - 1 + height) * d_stride + i + x];
                }

                H[(i * wiener_win + k) * wiener_win2 + i * wiener_win + l] = H[(i * wiener_win + k - 1) * wiener_win2 + i * wiener_win + l - 1] + delta;
            }
        }
    }
    ...
    // Divided into 4x4 squares to do load-and-transpose-and-store in optimization.
    // Demostration of TAP 3:
    //  \ lj   00    10    20    01      11    21    02    12      22
    // ki -------------------------------------------------------------
    // 00 |                                                           |
