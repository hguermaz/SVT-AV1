...
#if MD_INJECTION
#if MEMORY_FOOTPRINT_OPT_ME_MV
                    context_ptr->mdc_candidate_ptr->motion_vector_xl1 = me_results->me_mv_array[cuIndexInRaterScan][((sequence_control_set_ptr->static_config.mrp_mode == 0) ? 4 : (sequence_control_set_ptr->static_config.mrp_mode == 1) ? 2 : 1)].x_mv << 1;
                    context_ptr->mdc_candidate_ptr->motion_vector_yl1 = me_results->me_mv_array[cuIndexInRaterScan][((sequence_control_set_ptr->static_config.mrp_mode == 0) ? 4 : (sequence_control_set_ptr->static_config.mrp_mode == 1) ? 2 : 1)].y_mv << 1;
#else
