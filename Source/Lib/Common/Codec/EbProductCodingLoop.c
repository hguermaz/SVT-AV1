	...
	uint64_t                     *y_full_distortion)
{
	...
	for (context_ptr->tx_depth = 0; context_ptr->tx_depth <= end_tx_depth; context_ptr->tx_depth++) {
		...
		*y_coeff_bits = 0;
		...
		for (context_ptr->txb_itr = 0; context_ptr->txb_itr < txb_count; context_ptr->txb_itr++) {
			...
			uint32_t input_tu_origin_index = (context_ptr->sb_origin_x + tx_org_x + input_picture_ptr->origin_x) + 
			...
			txb_1d_offset += curr_tx_w * curr_tx_h;
	...
	free(tuFullDistortion[2]);
	free(tuFullDistortion[1]);
	free(tuFullDistortion[0]);
	free(tuFullDistortion);
}
#endif
