...
const EB_PREDICTION_FUNC  ProductPredictionFunTable[3] = 
{   NULL, 
    inter_pu_prediction_av1, /*INTER */
    AV1IntraPredictionCL     /*INTRA */
};
                ...
                if (context_ptr->best_uv_cost[candidateBuffer->candidate_ptr->intra_luma_mode] < cfl_uv_cost) {
                    ...
                    candidateBuffer->candidate_ptr->transform_type[PLANE_TYPE_UV] =
                        av1_get_tx_type(
                            ...
                            0,
                            (PredictionMode)NULL,
                            (UV_PredictionMode)context_ptr->best_uv_mode[candidateBuffer->candidate_ptr->intra_luma_mode],
                            PLANE_TYPE_UV,
                            0,
                            0,
                            0,
    ...
    ModeDecisionContext_t   *context_ptr)
{
    ...
    for (uv_mode = uv_mode_start; uv_mode <= uv_mode_end; uv_mode++) {
        ...
        //uint8_t angleDeltaCandidateCount = (use_angle_delta && av1_is_directional_mode(uv_mode)) ? 7 : 1; // TBD
        //for (int8_t angleDeltaCounter = 0; angleDeltaCounter < angleDeltaCandidateCount; ++angleDeltaCounter) // TBD
        {
    ...
    EbBool angle_search = (context_ptr->blk_geom->bsize >= BLOCK_8X8);
